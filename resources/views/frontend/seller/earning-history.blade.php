            ...
            <div class="col-lg-12">
                <div class="panel panel-default">
                    ...
                    <div class="panel-body">
                        @if(count($designerEarningHistories) > 0 )
                            <div class="table-responsive">
                                <table class="table table-bordered table-striped table-hover datatable datatable-SubCompany" id="subCompany-dataTable">
                                    ...
                                    <tbody>
                                    @foreach($designerEarningHistories as $index => $designerEarningHistory)
                                        <tr>
                                            <td>{{ $index + 1 }}</td>

                                            @if($designerEarningHistory->product_id)
                                            <td>
                                                <a target="_blank" href="{{ route("product-details",$designerEarningHistory->product->id) }}">{{ $designerEarningHistory->product->title }}</a></td>
                                            <td class="text-center align-middle" style="width: 120px;">
                                                <div class="position-relative overflow-hidden rounded shadow-sm d-flex align-items-center justify-content-center"
                                                    ...
                                                    @if($designerEarningHistory->product->type == 1)
                                                        <img src="{{ route('product.file.view', $designerEarningHistory->product->id) }}" alt="Image"
                                                    ...
                                                    @else
                                                        <video style="width: 100%; height: 100%; object-fit: cover; border-radius: 3px;" muted onmouseover="this.play()" onmouseout="this.pause()">
                                                            <source src="{{ route('product.view.video', $designerEarningHistory->product->id) }}" type="{{ $designerEarningHistory->product->file_type }}">
                                                        </video>
                                                        <div class="position-absolute top-50 start-50 translate-middle d-flex align-items-center justify-content-center shadow"
                                                             style="width: 32px; height: 32px; background: rgba(0,0,0,0.6); border-radius: 50%; color: #fff;">
                                                        ...
                                                        <div class="modal-content border-0 rounded-0" style="background: rgba(0,0,0,0.85); display: flex; justify-content: center; align-items: center; min-height: 100vh;">
                                                            <div class="d-flex flex-column overflow-hidden shadow-lg m-auto position-relative" style="width: 90vw; height: 90vh; border-radius: 4px; background: #111;">
                                                                ...
                                                                <div class="flex-grow-1 position-relative d-flex justify-content-center align-items-center overflow-hidden w-100 h-100" style="background: #111;">
                                                                    @if($designerEarningHistory->product->type == 1)
                                                                        <img src="{{ route('product.file.view', $designerEarningHistory->product->id) }}" alt="{{ $designerEarningHistory->product->title }}" class="w-100 h-100 d-block" style="object-fit: contain;">
                                                                    @else
                                                                ...
                                                                <div class="d-flex justify-content-between align-items-center" style="background: #2a2c31; padding: 12px 24px;">
                                                                    ...
                                                                    <div class="text-end" style="font-size: 11px; font-weight: 600; color: #ffffff; line-height: 1.4;">
                                                                        IMAGE ID: {{ $designerEarningHistory->product->asset_id ?? '' }}<br>
                                            ...
                                            @else
                                                <td> <a target="_blank" href="{{ route("customize-details",$designerEarningHistory->project->id) }}">{{ $designerEarningHistory->project->name }}</a></td>
                                                <td>  <span class="badge bg-secondary">Order Project</span></td>

                                            @endif

                                            <td> {{ $designerEarningHistory->user?->name ?? 'N/A' }}</td>
                                            <td> Tk. <strong>{{ number_format($designerEarningHistory->amount ?? 0, 2)  }}</strong> </td>

                                            <td> {{ $designerEarningHistory->card_type ?? 'N/A' }}</td>
                                            <td> {{ $designerEarningHistory->bank_txn ?? 'N/A' }}</td>
                                            <td>{{ \Carbon\Carbon::parse($designerEarningHistory->created_at)->format('d-m-Y') }}</td>
                                        </tr>
                            ...
                            <div class="pagination-wrapper d-flex justify-content-center mt-4">
                                {{ $designerEarningHistories->withQueryString()->links('pagination.custom') }}
                            </div>
                        @else
                            <div class="col-12 text-center py-4">
                                <p class="mb-0 text-danger">No Earning available yet.</p>
                            </div>
                        @endif
                    </div>
                </div>
