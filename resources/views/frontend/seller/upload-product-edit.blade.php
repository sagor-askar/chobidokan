                ...
                <div class="upload-card" id="uploadForm">
                    <div class="section-header">
                        <div class="section-title">
                            ...
                            <div>
                                <h4 class="mb-1 font-weight-bold" style="color: #0f172a;">Update Your Design</h4>
                                <p class="text-muted small mb-0">Modify the information and media files for this design asset.</p>
                            </div>
                        </div>
                    </div>

                    @if(session('success'))
                        <div class="alert alert-success alert-dismissible fade show mb-4 border-0 rounded-3 shadow-sm" role="alert" style="background-color: #d1fae5; color: #065f46;">
                    ...
                    <form action="{{ route('designer.products.update', $product->id) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-12">
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control @error('title') is-invalid @enderror"
                                           id="designTitle" name="title" value="{{ old('title', $product->title) }}" placeholder="Enter the title" required>
                                    @error('title')
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <select id="type-select" name="type" class="form-select @error('type') is-invalid @enderror" required>
                                        ...
                                        <option value="1" {{ old('type', $product->type) == 1 ? 'selected' : '' }}>🖼️ Image</option>
                                ...
                                <div class="free-toggle-wrapper">
                                    <div class="d-flex align-items-center gap-2" style="margin-left: 10px;" >
                                        <div class="form-check form-switch mb-0">
                                            <input class="form-check-input" type="checkbox" role="switch" id="is_free" name="is_free" value="1" {{ (old('_token') ? old('is_free') == '1' : $product->is_free == 1) ? 'checked' : '' }}>
                                        </div>
                                    </div>
                            ...
                            <div class="col-md-6 mb-3" id="price-container" style="display: none;">
                                ...
                                <div class="position-relative">
                                    <select id="price-select" class="form-select @error('price') is-invalid @enderror" disabled style="background-color: #e9ecef; cursor: not-allowed;">
                                        ...
                                        <option value="{{ (float)($settings->image_price ?? 0) }}">Image Price - {{ $settings->image_price ?? 0 }}</option>
                                        <option value="{{ (float)($settings->video_price ?? 0) }}">Video Price - {{ $settings->video_price ?? 0 }}</option>
                                    </select>
                            ...
                            <div class="col-12">
                                <div class="mb-3">
                                    ...
                                    <textarea class="form-control @error('description') is-invalid @enderror"
                                              id="description" name="description" rows="3" placeholder="Provide a brief overview of your design...">{{ old('description', $product->description) }}</textarea>
                                    @error('description')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <div class="col-12">
                                <div class="mb-4">
                                    <label class="form-label">Upload File <span class="text-muted fw-normal">(Optional - leave empty to keep current file)</span></label>
                                    <div class="dropzone-box" onclick="document.getElementById('designFile').click();">
                                        <div class="dropzone-icon">
                                            <i class="bi bi-cloud-arrow-up"></i>
                                        </div>
                                        <h6 class="fw-bold mb-1" style="color: #1e293b;">Click to upload or drag & drop new file</h6>
                                        ...
                                        <input type="file" class="d-none @error('file') is-invalid @enderror"
                                               id="designFile" name="file" accept="image/*,video/*">
                                    </div>
                                    <div class="file-preview-info" id="file-preview-box">
                                        <i class="bi bi-file-earmark-check-fill text-success fs-5"></i>
                                        <span id="file-name-text">No new file selected</span>
                                        <span class="badge bg-success ms-auto">New file loaded</span>
                                    </div>
                                        ...
                                        <div class="mt-3 p-3 bg-light rounded-3 border">
                                            <div class="d-flex align-items-center gap-3">
                                                <div class="position-relative overflow-hidden rounded shadow-sm d-flex align-items-center justify-content-center"
                                                     style="width: 100px; height: 75px; flex-shrink: 0; cursor: pointer; border: 1px solid #cbd5e1; background: #f8f9fa;">
                                                    @if($product->type == 1)
                                                        ...
                                                        <div class="position-absolute top-50 start-50 translate-middle d-flex align-items-center justify-content-center shadow"
                                                             style="width: 28px; height: 28px; background: rgba(0,0,0,0.6); border-radius: 50%; color: #fff;">
                                                    ...
                                                    @else
                                                        ...
                                                        <div class="position-absolute top-50 start-50 translate-middle d-flex align-items-center justify-content-center shadow"
                                                             style="width: 28px; height: 28px; background: rgba(0,0,0,0.6); border-radius: 50%; color: #fff;">
                                                            <i class="bi bi-play-fill" style="font-size: 16px; margin-left: 2px;"></i>
                                                        </div>
                                                    @endif
                                                </div>
                                                <div>
                                                    <h6 class="fw-bold text-dark mb-1" style="font-size: 0.95rem;">
                                                        @if($product->type == 1) Current Image File @else Current Video Footage @endif
                                                    </h6>
                                                    <p class="text-muted small mb-0 text-break">{{ basename($product->file_path) }}</p>
                                                </div>
                                            </div>
                                        </div>
                                    @endif
                        ...
                        <div class="d-flex justify-content-end pt-2">
                            <button type="submit" class="btn btn-submit-gradient w-100 justify-content-center py-3">
                                <i class="bi bi-check-circle-fill"></i> Update Asset Info
                            </button>
                        </div>
                    </form>
...
<script>
    // Tags Input Handling & Population from Database
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const imagePrice = parseFloat("{{ $settings->image_price ?? 0 }}");
        const videoPrice = parseFloat("{{ $settings->video_price ?? 0 }}");
