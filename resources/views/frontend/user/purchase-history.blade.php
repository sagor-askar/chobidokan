                        ...
                        @if(count($purchaseHistories) > 0 )
                            <div class="table-responsive">
                                <table class="table table-bordered table-striped table-hover datatable datatable-SubCompany" id="subCompany-dataTable">
                                    ...
                                    <tbody>
                                    @foreach($purchaseHistories as $index => $purchaseHistory)
                                        <tr>
                                            <td>{{ $index + 1 }}</td>

                                            @if($purchaseHistory->product_id)
                                            <td>
                                                <a target="_blank" title="Details" href="{{ route('product-details',$purchaseHistory->product?->id) }}">{{ $purchaseHistory->product?->title }}</a>
                                            </td>
                                                <td class="text-center align-middle" style="width: 120px;">
                                                    <div class="position-relative overflow-hidden rounded shadow-sm d-flex align-items-center justify-content-center"
                                                        ...
                                                        @if($purchaseHistory->product->type == 1)
                                                            <img src="{{ route('product.file.view', $purchaseHistory->product->id) }}" alt="Image"
                                                        ...
                                                        @else
                                                            <video style="width: 100%; height: 100%; object-fit: cover; border-radius: 3px;" muted onmouseover="this.play()" onmouseout="this.pause()">
                                                                <source src="{{ route('product.view.video', $purchaseHistory->product->id) }}" type="{{ $purchaseHistory->product->file_type }}">
                                                            </video>
                                                            <div class="position-absolute top-50 start-50 translate-middle d-flex align-items-center justify-content-center shadow"
                                                                 style="width: 32px; height: 32px; background: rgba(0,0,0,0.6); border-radius: 50%; color: #fff;">
                                                    ...
                                                    <div class="modal fade" id="mediaModal{{ $index }}" tabindex="-1" aria-hidden="true" style="z-index: 9999;">
                                                        <div class="modal-dialog modal-fullscreen m-0" style="max-width: 100vw; height: 100vh;">
                                                            <div class="modal-content border-0 rounded-0" style="background: rgba(0,0,0,0.85); display: flex; justify-content: center; align-items: center; min-height: 100vh;">
                                                                ...
                                                                <div class="d-flex flex-column overflow-hidden shadow-lg m-auto" style="width: 90vw; height: 90vh; border-radius: 4px; background: #111;">
                                                                    <div class="flex-grow-1 position-relative d-flex justify-content-center align-items-center overflow-hidden w-100 h-100" style="background: #111;">
                                                                        @if($purchaseHistory->product->type == 1)
                                                                            <img src="{{ route('product.file.view', $purchaseHistory->product->id) }}" alt="{{ $purchaseHistory->product->title }}" class="w-100 h-100 d-block" style="object-fit: contain;">
                                                                        @else
                                                                    ...
                                                                    <div class="d-flex justify-content-between align-items-center" style="background: #2a2c31; padding: 12px 24px;">
                                                                        ...
                                                                        <div class="text-end" style="font-size: 11px; font-weight: 600; color: #ffffff; line-height: 1.4;">
                                                                            IMAGE ID: {{ $purchaseHistory->product->asset_id ?? '' }}<br>
                                            ...
                                            @else
                                                <td>
                                                    <a target="_blank" title="Details" href="{{ route('pricing-info') }}">{{ $purchaseHistory->subscription?->name  }}</a>
                                                </td>
                                                <td>  <span class="badge bg-danger"> Subscription Plan </span></td>
                                            @endif
                                            <td> TK. {{ $purchaseHistory->amount }}</td>
                                            <td> {{ $purchaseHistory->card_type }}</td>
                                            <td>{{ \Carbon\Carbon::parse($purchaseHistory->created_at)->format('d-m-Y') }}</td>
                                            <td>
                                                @if($purchaseHistory->product_id)
                                                    @if($purchaseHistory->product->type ==1 )
                                                        <a href="{{ route('product.image-download', ['id' => base64_encode($purchaseHistory->product?->id)]) }}" class="btn btn-primary btn-sm d-inline-flex align-items-center gap-1" title="Download">
                                                            <i class="fa fa-download"></i> Download
                                                        </a>
                                                    @else
                                                        <a href="{{ route('product.video-download', ['id' => base64_encode($purchaseHistory->product?->id)]) }}" class="btn btn-primary btn-sm d-inline-flex align-items-center gap-1" title="Download">
                                                            <i class="fa fa-download"></i> Download
                                                        </a>
                            ...
                            <div class="pagination-wrapper d-flex justify-content-center mt-4">
                                {{ $purchaseHistories->withQueryString()->links('pagination.custom') }}
                            </div>
                        @else
